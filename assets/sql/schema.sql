...
('# Crashers on <version_title>', '3', 'rgb(51, 51, 51)', 
    '{"from": "public_bugs","select": {"name": "num","value": "bug_id","aggregate": "count"},"esfilter": {"and": [{"term": {"cf_tracking_firefox<version_tag>": "+"}},{"terms": {"keywords": ["crash","topcrash","topcrash-mac","topcrash-win"]}}]},"edges": [{"range": {"min": "modified_ts","max": "expires_on"},"domain": {"type": "date","min": <birthday>,"max": <timestamp>,"interval": "day"}}]}',
...
('# Regressions on <version_title>', '4', 'rgb(51, 51, 51)', 
    '{"from": "public_bugs","select": {"name": "num","value": "bug_id","aggregate": "count"},"esfilter": {"and": [{"term": {"cf_tracking_firefox<version_tag>": "+"}},{"terms": {"keywords": ["regression"]}},{"terms": {"bug_status": ["unconfirmed","new","assigned","reopened"]}}]},"edges": [{"range": {"min": "modified_ts","max": "expires_on"},"domain": {"type": "date","min": <birthday>,"max": <timestamp>,"interval": "day"}}]}',
