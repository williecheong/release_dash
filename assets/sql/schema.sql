...
('# Crashers on <version_title>', '3', 'rgb(51, 51, 51)', 
    '{"from": "private_bugs","select": {"name": "num","value": "bug_id","aggregate": "count"},"esfilter": {"and": [{"term": {"cf_tracking_firefox<version_tag>": "+"}},{"terms": {"keywords": ["crash","topcrash","topcrash-mac","topcrash-win"]}},{"missing":{"field": "bug_group"}   }]},"edges": [{"range": {"min": "modified_ts","max": "expires_on"},"domain": {"type": "date","min": <birthday>,"max": <timestamp>,"interval": "day"}}]}',
